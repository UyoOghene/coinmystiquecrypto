        // Full interactive script with working theme toggle
        const CONFIG = { apiBase: 'https://api.coingecko.com/api/v3', updateInterval: 30000, defaultCoins: ['bitcoin','ethereum','litecoin'] };
        let state = { selectedCoins: new Set(CONFIG.defaultCoins), selectedCurrency: 'usd', autoInterval: null, priceData: {}, chart: null, isDark: false };
        const els = {
            ...
            totalMarketCap: document.getElementById('totalMarketCap'), totalVolume: document.getElementById('totalVolume'), btcDominance: document.getElementById('btcDominance'), 
            activeCoins: document.getElementById('activeCoins'), marketCapChange: document.getElementById('marketCapChange')
        };
        ...
        async function fetchGlobal() {
            try { const res = await fetch(`${CONFIG.apiBase}/global`); const data = await res.json(); if(data.data){ 
                els.totalMarketCap.textContent = formatCurrency(data.data.total_market_cap?.[state.selectedCurrency]||0, state.selectedCurrency); 
                els.totalVolume.textContent = formatCurrency(data.data.total_volume?.[state.selectedCurrency]||0, state.selectedCurrency); 
                els.btcDominance.textContent = `${(data.data.market_cap_percentage?.btc||0).toFixed(1)}%`; 
                const ch = data.data.market_cap_change_percentage_24h_usd||0; 
                els.marketCapChange.textContent = `${ch>=0?'+':''}${ch.toFixed(2)}%`; 
            } } catch(e){} 
        }
        ...
        async function searchCoin() {
            ...
            try { 
                const url = `${CONFIG.apiBase}/simple/price?ids=${coin}&vs_currencies=${state.selectedCurrency}&include_24h_change=true`; 
                const res = await fetch(url); const data = await res.json(); if(!data[coin]) throw new Error(); 
                const p = data[coin][state.selectedCurrency]; const ch = data[coin][`${state.selectedCurrency}_24h_change`]||0; 
                els.searchResult.innerHTML = `<div><i class="fas fa-coins" style="font-size:2rem;"></i><h3>${formatCoin(coin)}</h3><div style="font-size:1.5rem;">${formatCurrency(p,state.selectedCurrency)}</div><div class="price-change ${ch>=0?'positive':'negative'}">${ch.toFixed(2)}%</div><button class="btn-action btn-primary" style="margin-top:12px;" onclick="addCoin('${coin}')"><i class="fas fa-plus"></i> Track</button></div>`; 
            } catch(e){ els.searchResult.innerHTML = '<p style="color:#ef4444;">Coin not found</p>'; } 
        }
        
        window.addCoin = (c) => { 
            if(!state.selectedCoins.has(c)){ 
                state.selectedCoins.add(c); 
                const opt = document.createElement('option'); 
                opt.value = c; opt.textContent = `${formatCoin(c)} (${c.toUpperCase()})`; 
                opt.selected = true; els.coinSelect.appendChild(opt); 
                fetchPrices(); 
                els.searchResult.innerHTML = `<p style="color:#10b981;">✓ ${formatCoin(c)} added!</p>`; 
            } 
        };
        ...
        async function fetchHistory(coin=els.chartCoin.value, days=els.chartInterval.value) { 
            try { 
                const url = `${CONFIG.apiBase}/coins/${coin}/market_chart?vs_currency=${state.selectedCurrency}&days=${days}`; 
                const res = await fetch(url); const data = await res.json(); if(!data.prices) return; 
                const prices = data.prices.slice(-24*days); 
                const labels = prices.map(([ts])=> days==1?new Date(ts).toLocaleTimeString([],{hour:'2-digit'}):new Date(ts).toLocaleDateString()); 
                const vals = prices.map(([,p])=>p); 
                const ctx = els.priceChart.getContext('2d'); 
                if(state.chart) state.chart.destroy(); 
                state.chart = new Chart(ctx, { type:'line', data:{ labels, datasets:[{ label:`${formatCoin(coin)} price`, data:vals, borderColor:'#6366f1', backgroundColor:'rgba(99,102,241,0.1)', borderWidth:2, fill:true, tension:0.3 }] }, options:{ responsive:true, maintainAspectRatio:false, plugins:{ tooltip:{ callbacks:{ label:(ctx)=>formatCurrency(ctx.raw,state.selectedCurrency) } } }, scales:{ y:{ ticks:{ callback:(v)=>formatCurrency(v,state.selectedCurrency) } } } } }); 
            } catch(e){} 
        }
        ...
        // FIXED: Proper theme toggle functionality
        function toggleTheme() { 
            const body = document.body;
            if (body.classList.contains('dark-theme')) {
                body.classList.remove('dark-theme');
                els.themeToggle.innerHTML = '<i class="fas fa-moon"></i><span> Dark Mode</span>';
                state.isDark = false;
            } else {
                body.classList.add('dark-theme');
                els.themeToggle.innerHTML = '<i class="fas fa-sun"></i><span> Light Mode</span>';
                state.isDark = true;
            }
        ...
        els.coinSearch.addEventListener('keypress', (e) => { if(e.key === 'Enter') searchCoin(); });
        ...
        document.querySelectorAll('.nav-btn, .footer-links a').forEach(anchor => {
